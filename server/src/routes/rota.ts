...
import { RotaServiceType, RotaRole } from "@prisma/client";
...
const VALID_SERVICE_TYPES: RotaServiceType[] = ["sunday", "wednesday"];
const VALID_ROLES: RotaRole[] = [
  "opening_prayer",
  "rhapsody_reading",
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    // role must be valid; keep only rows where (user_id OR name) exists; no duplicates by role
    const seenRoles = new Set<RotaRole>();
    const cleaned: { role: RotaRole; userId: number | null; name: string | null }[] = [];

    for (const a of payloadAssignments) {
      if (!a?.role || !VALID_ROLES.includes(a.role)) continue;
    ...
    try {
      ...
      if (String(e?.code) === "P2002") {
        return res
          .status(409)
          .json({ error: "Rota already exists for this date and service type" });
      }
...
router.put(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const { service_date, service_type, notes, assignments } = req.body as {
      service_date?: string;
      service_type?: RotaServiceType; // ✅ FIXED (was ServiceType)
      notes?: string;
      assignments?: { role: RotaRole; user_id?: number; name?: string }[];
    };
    ...
    let cleanedAssignments:
      | { role: RotaRole; userId: number | null; name: string | null }[]
      | null = null;
          ...
          if (cleanedAssignments.length > 0) {
            await tx.rotaAssignment.createMany({
              data: cleanedAssignments.map((a) => ({
                ...
                role: a.role,
                userId: a.userId,
                name: a.name, // ✅ FIXED: keep typed names
              })),
            });
          }
        }
