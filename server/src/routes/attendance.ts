...
router.get('/date/:date', authenticateToken, async (req: AuthRequest, res: Response) => {
  ...
  const where: { serviceDate: Date; serviceType?: ServiceType } = { serviceDate: new Date(date) };
...
router.get('/stats', authenticateToken, async (req: AuthRequest, res: Response) => {
  ...
  // Overall attendance by service type (attended is Boolean, so we COUNT, not SUM)
  ...
  const attendedMap = new Map(attendedTotals.map((s) => [s.serviceType, s._count._all]));
  ...
  const lowAttendance = members
    .map((m) => {
      ...
      return {
        ...
        attendance_rate: totalServices > 0 ? Math.round((attendedCount / totalServices) * 100) : null,
      };
    })
